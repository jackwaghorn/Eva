// Eye animation
const $element = $('.eye-animation, .eye-news, .eye-listen');
const imagePath = './img/lips';
...
function step(startTime) {
  ...
  if (timeFromLastUpdate > timePerFrame) {
    ...
    $element.attr('src', imagePath + `/lips_${frameNumber}.png`);
...
$(document).ready(() => {
  for (var i = 1; i < totalFrames + 1; i++) {
    $('body').append(`<div id="preload-image-${i}" style="background-image: url('${imagePath}/lips_${i}.png');"></div>`);
  }
});
